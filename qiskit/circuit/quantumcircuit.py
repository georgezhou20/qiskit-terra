...
class QuantumCircuit:
    ...
    def add_register(self, *regs):
        ...
        for register in regs:
            ...
            if isinstance(register, QuantumRegister):
                ...
                self._qubits.extend(bit for bit in register
                                    if bit not in existing_qubits)
            elif isinstance(register, ClassicalRegister):
                self.cregs.append(register)
                existing_clbits = set(self.clbits)
                self._clbits.extend(bit for bit in register
                                    if bit not in existing_clbits)
    ...
    def _check_qargs(self, qargs):
        ...
        if not set(qargs).issubset(self.qubits):
    ...
    def _check_cargs(self, cargs):
        ...
        if not set(cargs).issubset(self.clbits):
